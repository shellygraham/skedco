 ...
 */
 ...
 get_header();
 global $woo_options;
 $woo_options['sidebar_override'] = 'search';
 ...
 if ($_GET['redirect']) {
 	$redirects = get_option('popart_sample');
 	foreach (explode("\n",$redirects['pairs']) as $pair) {
 		$r = explode(':',$pair);
		if (strtolower( get_search_query()) == $r[0]) {
			$redirect_to = home_url() . '/' . $r[1];
    ...
    <div id="content" class="col-full">
		...
		<?php if ( have_posts() ) : $count = 0; ?>
			...
			<?php
				if ($_GET['redirect']) {
					// SKU match
					$terms = get_the_terms( $product->id, 'pa_nsn');
					
					$sku = strtolower(get_post_meta( get_the_ID(), '_sku', TRUE ));
					if ( $sku == strtolower( get_search_query() ) ) {
						wp_redirect(get_permalink(get_the_ID()), 301 ); exit;
					}
					
					// Not sure about this one...
					$nsn = current($terms)->name;
					if ( $nsn == strtolower( get_search_query() ) ) {
						echo "<!-- NSN Highlight -->";
					}
				}
			?>
